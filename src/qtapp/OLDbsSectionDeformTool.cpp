...
bsSectionDeformTool::bsSectionDeformTool(bsEditorWindow2D* editor,
					 QWidget* parent,
					 const char* name,
					 QPixmap* pixmap,
					 QBitmap* bitmask):
    bsTool(editor, parent, name, pixmap, bitmask) {
...
void bsSectionDeformTool::activate(bool b) {
    ...
    bsSectionViewer2D* v = m_editor->getViewer();
...
int bsSectionDeformTool::getDeformVertex(const sl::point2f& p) const {
    ...
    double d_min = 1e30;
    int i_min = -1;
    for (int i=0; i< m_current_deformed_vertices.size(); i++) {
	sl::point2f p_i = worldToNormalized(m_current_deformed_vertices[i]);
	double d = distSquared(p, p_i);
	if (d<d_min) {
	    d_min = d;
	    i_min = i;
	}
    ...
    if (i_min >= 0 && (d_min < pow(2.0*click_radius/(float)m_editor->getViewer()->width(), 2.0f))) {
	result = i_min;
    }
    return result;
}
...
void bsSectionDeformTool::handleMouseMove(QMouseEvent *e) {
    ...
    case STATUS_DEFORM_VERTEX:
	{
	    assert(m_current_deform_vertex >= 0 && m_current_deform_vertex < m_current_deformed_vertices.size());
	    sl::point2f pn_mouse  = pixelsToNormalized(e);
	    deform(m_current_deform_vertex, normalizedToWorldNear(pn_mouse));
	    updateGL();
	}
...
void bsSectionDeformTool::init_deform() {
    ...
    m_current_deform_triangulation = bsGeometryHelpers::triangulation(tmpconnectivity,
								      tmpvertices);
    {
	int current_section = m_editor->getCurrentSectionIndex();
...
void bsSectionDeformTool::deform(int i, const sl::point3f& p_new) {
    assert(i>= 0 && i < m_current_deformed_vertices.size());
    m_current_deformed_vertices[m_current_deform_vertex] = p_new;
	...
	if (doc) {
		...
		bsSection* s = b->sectionItem(current_section);
		{
		    for (int i=0; i<m_current_deform_section_points.size(); i++) {
		...
		// NEURONS!!!
		{
		    for (int tp=0; tp<m_current_deform_section_neurons.size(); tp++) {
			for (int i=0; i<m_current_deform_section_neurons[tp].size(); i++) {
